...
void handle_char(char ***args, int *args_count, char *buffer, int *buffer_index, char current_char, char end_char, int *in_char) {
...
char **parse_args(const char *input) {
    ...
    while (index < input_len) {
        ...
        if (current_char == '\'' && input_len > index+1 && input[index+1] == '\'') {
            index++;
        } else if (in_quote == FALSE && in_space == FALSE) {
            while (input[index] == ' ') {
                index++;
            }
            if (input[index] == '\''){
